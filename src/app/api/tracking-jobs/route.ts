...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`${aiUrl}/tracking-jobs`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        max_frames: payload.maxFrames ?? 120
      })
    });
    ...
    return NextResponse.json({ error: `Could not start tracking: ${message}` }, { status: 503 });
  }
}
