...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`${aiUrl}/analyze-url`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `AI service returned ${response.status}: ${detail || "No details"}` },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json({ error: `Could not reach AI service: ${message}` }, { status: 503 });
  }
}
